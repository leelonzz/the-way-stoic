...
async function handleSubscriptionEvent(event: PayPalWebhookEvent): Promise<void> {
  const { event_type, resource } = event;
  const subscriptionId = resource.id;
  const userId = resource.custom_id;
  const planId = resource.plan_id;

  console.log(`Processing ${event_type} for subscription ${subscriptionId}, user ${userId}`);
  ...
  try {
    switch (event_type) {
      ...
      case 'PAYMENT.SALE.COMPLETED':
        ...
        console.log(`Payment completed for subscription ${subscriptionId}, user ${userId}`);
        break;

      case 'PAYMENT.SALE.DENIED':
        await incrementFailedPayments(subscriptionId);
        console.log(`Payment denied for subscription ${subscriptionId}, user ${userId}`);
        break;
...
async function recordPayment(subscriptionId: string, paymentInfo?: unknown): Promise<void> {
  const { error } = await supabase
    .from('subscriptions')
    .update({

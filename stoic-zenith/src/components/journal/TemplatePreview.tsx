...
interface TemplatePreviewProps {
  ...
  onUseNow: (template: JournalTemplate) => void
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-50 text-gray-500">
        <div className="text-center">
          ...
          <p className="text-sm mt-1">Choose a template from the left to preview it here</p>
        </div>
      </div>
    )
  }
    ...
    switch (block.type) {
      ...
      case 'paragraph':
        if (block.text.includes('TODAY I AM GRATEFUL FOR') || 
          ...
          return (
            <div key={index} className={`${baseClasses} bg-gray-100 px-3 py-2 rounded-md`}>
        ...
        if (block.text.trim() && block.text.includes(':')) {
          return (
            <p key={index} className={`${baseClasses} font-medium text-gray-800`}>
              {block.text}
            </p>
          )
        }
    ...
    <div className="flex-1 flex flex-col bg-white">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="max-w-2xl">
          {template.template_content.blocks.map((block, index) => renderBlock(block, index))}
        </div>
      </div>

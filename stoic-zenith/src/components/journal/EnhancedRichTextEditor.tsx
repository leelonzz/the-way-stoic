...
export const EnhancedRichTextEditor = React.memo(
  function EnhancedRichTextEditor({
    ...
    const focusBlockElement = useCallback(
      (
          ...
          if (position === 'end') {
            ...
            if (
              ...
              if (
                ...
                contentEditable.textContent.length > 0
              ) {
    ...
    const focusBlock = useCallback(
      (blockId: string, position: 'start' | 'end' = 'start') => {
        const timeout = setTimeout(() => {
          ...
          if (!blockElement) {
            ...
            const retryTimeout = setTimeout(() => {
              const retryBlockElement = document.querySelector(
                `[data-block-id="${blockId}"]`
              ) as HTMLElement
              ...
              retryBlockElement.scrollIntoView({
                behavior: 'smooth',
                block: 'nearest',
              })
    ...
    const getCurrentLineInfo = useCallback(
      (contentEditable: HTMLElement, selection: Selection) => {
        ...
        const textBeforeCursor = currentLineText.substring(
          0,
    ...
    const isAtStartOfLine = useCallback(
      (contentEditable: HTMLElement, selection: Selection): boolean => {
        ...
        if (selectionStartBlock !== currentBlock) {
          ...
          allContentEditables.forEach(el => {
            if (!originalContentEditableStates.has(el)) {
              originalContentEditableStates.set(
                el,
    ...
    const handleCrossBlockDeletion = (): void => {
      ...
      if (firstIndex === lastIndex) {
        ...
        if (firstBlockContentEditable && lastBlockContentEditable) {
          ...
          const firstBlockText = firstBlockFullText.substring(
            0,
            firstBlockEndPos
          )
    ...
    const handleBlockKeyDown = useCallback(
      (e: KeyboardEvent, blockId: string): void => {
        ...
        // Handle Enter key for list continuation, heading blocks, and image blocks
        if (e.key === 'Enter' && !showCommandMenu) {
          ...
          // CRITICAL FIX: Handle heading blocks - create new paragraph block after them
          if (block.type === 'heading') {
            e.preventDefault()

            // Get current cursor position to split text properly
            const selection = window.getSelection()
            const contentEditable = (e.target as HTMLElement).closest(
              '[contenteditable]'
            ) as HTMLElement
            ...
            if (selection && selection.rangeCount > 0 && contentEditable) {
              ...
              // Update current heading block with text before cursor
              updateBlock(blockId, {
                text: textBeforeCursor,
                richText: textBeforeCursor,
              })

              // Create a new PARAGRAPH block (not heading) with text after cursor
              const newBlockId = addBlock(blockId, {
                type: 'paragraph', // Always create paragraph after heading
                text: textAfterCursor,
                richText: textAfterCursor,
              })

              // CRITICAL FIX: Ensure cursor moves to new paragraph block
              ...
              // Focus the new paragraph block with multiple retry attempts
              const focusNewBlock = () => {
                ...
                if (newBlockElement) {
                  ...
                  if (contentEditable) {
                    // Direct focus approach for immediate response
                    contentEditable.focus()
                    ...
                    // Also use the focusBlock function as backup
                    focusBlock(newBlockId, 'start')
                    return true
                  }
                }
              ...
              // FALLBACK: Create a new paragraph block even if cursor position detection fails
              const newBlockId = addBlock(blockId, {
                type: 'paragraph',
                text: '',
                richText: '',
              ...
              // CRITICAL FIX: Clear current selection and focus new block
              const currentSelection = window.getSelection()
              if (currentSelection) {
                currentSelection.removeAllRanges()
              }
          ...
          if (block.type === 'bullet-list' || block.type === 'numbered-list') {
            ...
            if (selection && selection.rangeCount > 0 && contentEditable) {
              ...
              setTimeout(() => {
                ...
                if (newBlockElement) {
                  const newContentEditable = newBlockElement.querySelector(
                    '[contenteditable]'
                  ) as HTMLElement
        ...
        if (e.key === ' ') {
          ...
          if (selection && selection.rangeCount > 0 && contentEditable) {
            ...
            if (lineInfo && lineInfo.isAtLineStart) {
              ...
              const textBeforeSpace = lineInfo.lineText.substring(
                0,
                lineInfo.cursorPositionInLine
              )
              ...
              if (pattern) {
                ...
                const textAfterPattern = lineInfo.lineText.substring(
                  lineInfo.cursorPositionInLine
                )
                ...
                if (lineInfo.lineStart > 0) {
                  ...
                  const beforeLines = lineInfo.fullText.substring(
                    0,
                    lineInfo.lineStart - 1
                  ) // -1 to exclude the \n
                  const afterLines =
                    lineInfo.lineEnd < lineInfo.fullText.length
                  ...
                  // CRITICAL FIX: Clear the DOM content and reset cursor position for new block
                  setTimeout(() => {
                    const element = document.querySelector(
                      `[data-block-id="${newBlockId}"] [contenteditable]`
                    ) as HTMLElement
                    if (element) {
                      // Clear any markdown shortcut text from the DOM
                      element.innerHTML = ''
                      element.textContent = textAfterPattern || ''

                      // Clear any inherited formatting to ensure clean state
                  ...
                  // CRITICAL FIX: Clear the DOM content and reset cursor position
                  setTimeout(() => {
                    const element = document.querySelector(
              ...
              if (block.text.trim() === '') {
                ...
                if (blockIndex > 0) {
                  ...
                  updateBlock(prevBlock.id, {
                    ...
                    richText:
                      (prevBlock.richText || '') + (block.richText || ''),
                  })
                  deleteBlock(blockId)
                }
              }
        ...
        if (e.key === '/') {
          ...
          if (selection && selection.rangeCount > 0 && contentEditable) {
            ...
            if (
              lineInfo &&
              (lineInfo.cursorPositionInLine === 0 ||
    ...
    const handleSplashCommand = useCallback(
      (blockId: string, searchQuery: string): void => {
        console.log(
          '🚀 Splash command triggered from SimplifiedRichTextEditor:',
          { blockId, searchQuery }
        )
        ...
        if (command.type !== 'image') {
          setTimeout(() => {
            ...
            if (element) {
              ...
              // CRITICAL FIX: Clear any inherited formatting from contenteditable
              // This prevents heading formatting from carrying over to subsequent blocks
              if (document.execCommand) {
                // Remove all formatting to ensure clean state
                document.execCommand('removeFormat', false, '')
                document.execCommand('unlink', false, '')
              }
    ...
    const renderBlock = (block: JournalBlock, index: number): JSX.Element => {
      ...
      const showPlaceholder =
        index === 0 && (!block.text || block.text.trim() === '')

...
export const SimplifiedRichTextEditor = React.memo(
  function SimplifiedRichTextEditor({
    block,
    onChange,
    onKeyDown,
    onSplashCommand,
    ...
    const cursorPositionRef = useRef<{ start: number; end: number } | null>(
      null
    )
        ...
        if (onSplashCommand && editorRef.current) {
          ...
          if (selection && selection.rangeCount > 0) {
            try {
              ...
              // Check if current line starts with splash (improved detection)
              const trimmedLineText = currentLineText.trim().toLowerCase()
              if (trimmedLineText.startsWith('splash')) {
                // Extract search query after "splash" - handle both "splash" and "splash "
                let searchQuery = ''
                if (trimmedLineText.length > 6) {
                  // If there's content after "splash", extract it
                  const afterSplash = currentLineText.trim().substring(6).trim()
                  searchQuery = afterSplash
                }

                console.log(
                  '🎯 Splash command detected in SimplifiedRichTextEditor:',
                  {
                    blockId: block.id,
                    currentLineText,
                    trimmedLineText,
                ...
                // Always trigger splash command, even with empty search query
    ...
    const handleKeyDown = useCallback(
      (e: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        if (
          editorRef.current &&
          editorRef.current.hasAttribute('data-placeholder')
        ) {
    ...
    useEffect(() => {
      const handleSelectionChange = (): void => {
        ...
        if (hasSelection) {
          ...
          const range = selection.getRangeAt(0)
          const isWithinEditor = editorRef.current.contains(
    ...
    return (
      <div className="relative">
        ...
        <style jsx>{`
          ...
          /* CRITICAL FIX: Ensure each contenteditable block has isolated styling */
          [contenteditable] {
            /* Reset any inherited formatting */
          ...
          /* Prevent formatting inheritance between blocks */
          [contenteditable] * {
            font-size: inherit;
            font-weight: inherit;
            font-family: inherit;

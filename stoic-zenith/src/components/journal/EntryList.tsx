...
export const EntryList = React.memo(function EntryList({
  ...
  const processEntries = useCallback(
    (journalEntries: JournalEntry[]): EntryListItemData[] => {
      // DEDUPLICATION: Remove duplicate entries by ID (React key issue fix)
      const uniqueEntries = journalEntries.reduce(
        (acc: JournalEntry[], entry) => {
  ...
  const groupEntriesByDateRange = useCallback(
    (entries: EntryListItemData[]): GroupedEntries[] => {
      const now = new Date()
  ...
  const getDateDisplayForEntry = useCallback(
    (entry: JournalEntry, groupType: GroupedEntries['groupType']): string => {
      try {
        switch (groupType) {
          case 'today':
          case 'yesterday':
  ...
  return (
    <div
      className={`flex flex-col h-full overflow-hidden min-h-0 ${className}`}
    >
      {/* Simplified Header */}

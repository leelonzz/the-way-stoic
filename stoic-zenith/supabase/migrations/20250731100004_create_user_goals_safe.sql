-- Create user_goals table for personal stoic goals tracking (safe version)
...
-- Create function to handle goal updates (drop if exists first)
DROP FUNCTION IF EXISTS public.handle_goal_update();
...
CREATE INDEX IF NOT EXISTS idx_user_goals_completed ON public.user_goals(is_completed);
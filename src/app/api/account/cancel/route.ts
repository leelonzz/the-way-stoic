...
import {
  recordAccountCancellation,
  markTrialAsUsed,
} from '@/lib/trial-prevention'
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!cancellationResult.success) {
      console.error(
        'Failed to record account cancellation:',
    ...
    try {
      ...
      console.log(
        `✅ Real-time refresh triggered for account cancellation: ${user.id}`
      )
    } catch (refreshError) {
      console.error(
        'Failed to trigger real-time refresh for account cancellation:',
        refreshError
      )
    ...
    return NextResponse.json({
      success: true,
      message:
        'Account cancelled successfully. Your trial usage history has been preserved to prevent abuse.',

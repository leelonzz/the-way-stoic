...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      webhook.event_type === 'subscription.active' ||
      webhook.event_type === 'payment.succeeded'
    ) {
      ...
      result = {
        message: `Event type ${webhook.event_type} not implemented for recovery`,
      }
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const query = supabase.from('webhook_logs').delete()

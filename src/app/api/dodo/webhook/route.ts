...
import {
  ...
  emitWebhookCompletion,
  triggerSubscriptionUpdate,
} from '@/utils/subscriptionRefresh'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isTestWebhook =
      webhookHeaders['webhook-signature'] === 'test_signature_for_development'
    ...
    switch (event.type) {
      ...
      case 'subscription.active':
        await handleSubscriptionActive(
          event.data as unknown as SubscriptionEvent
        )
        break
      ...
      case 'subscription.on_hold':
        await handleSubscriptionOnHold(
          event.data as unknown as SubscriptionEvent
        )
        break

      case 'subscription.failed':
        await handleSubscriptionFailed(
          event.data as unknown as SubscriptionEvent
        )
        break
  ...
  try {
    ...
    if (payment.subscription_id && payment.metadata?.user_id) {
      ...
      if (userError || !user) {
        ...
        if (profileError) {
          console.error(
            'Failed to update user profile on payment success:',
            profileError
          )
        } else {
          console.log(
            `✅ User ${userId} upgraded to philosopher plan immediately on payment success`
          )
        }
      }
    ...
    if (payment.subscription_id && !payment.metadata?.user_id) {
      console.warn(
        `⚠️ Payment ${payment.id} has subscription but missing user_id in metadata`
      )
      return
    }
...
async function handlePaymentFailed(payment: PaymentEvent) {
  ...
  try {
    ...
    console.log(
      `❌ Payment ${payment.id} failed for customer ${payment.customer_id}`
    )
  } catch (error) {
...
async function handleSubscriptionActive(subscription: SubscriptionEvent) {
  ...
  try {
    ...
    if (!userId && subscription.customer_id) {
      console.log(
        'Method 2 - Looking up user by Dodo customer_id:',
    ...
    if (!userId && subscription.customer?.email) {
      console.log(
        'Method 3 - Looking up user by email:',
    ...
    if (!userId) {
      ...
      await supabase
        ...
        .update({
          error_message:
            'Could not find user for subscription activation - requires manual intervention',
    ...
    console.log(
      `✅ User ${userId} subscription ${subscription.id} activated successfully - profile updated`
    )
    ...
    try {
      ...
      if (emailResult.success) {
        console.log(
          `📧 Subscription confirmation email sent to ${subscription.customer?.email || user.user?.email}`
        )
      } else {
        console.error(
...
async function handleSubscriptionCancelled(subscription: SubscriptionEvent) {
  ...
  try {
    ...
    if (!userId) {
      console.log(
        'Method 4 - Looking up user by subscription_id:',
        subscription.id
      )
      const { data: profile } = await supabase
        .from('profiles')
    ...
    if (!userId) {
      ...
      await supabase
        ...
        .update({
          error_message:
            'Could not find user for subscription cancellation - requires manual intervention',
          error_details: errorDetails,
          processed_at: new Date().toISOString(),
          requires_manual_action: true,
        })
    ...
    console.log(
      `❌ User ${userId} subscription ${subscription.id} cancelled - immediately downgraded to seeker plan`
    )
    ...
    try {
      ...
      console.log(
        `✅ Marked trial as used for webhook cancelled subscription: ${userId}`
      )
    } catch (trialError) {
...
async function handleSubscriptionOnHold(subscription: SubscriptionEvent) {
  ...
  try {
    ...
    if (!userId) {
      console.error(
        'No user_id found in subscription metadata:',
    ...
    console.log(
      `⚠️ Subscription ${subscription.id} put on hold for user ${userId}`
    )
  } catch (error) {
...
async function handleSubscriptionFailed(subscription: SubscriptionEvent) {
  ...
  try {
    ...
    console.log(
      `❌ Subscription ${subscription.id} failed for user ${userId} - immediately downgraded to seeker plan`
    )

...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            'Please wait for your trial to expire before subscribing, or contact support to convert your trial to a paid subscription.',
        },
        { status: 409 }
      )
    }
      ...
      return NextResponse.json(
        {
          ...
          details:
            'Please manage your existing subscription instead of creating a new one.',
        },
        { status: 409 }
      )
    }
    ...
    const hasValidCustomerId =
      ...
      (profile.dodo_customer_id.startsWith('cust_') ||
        profile.dodo_customer_id.startsWith('customer_'))
    ...
    if (profile.dodo_customer_id && !hasValidCustomerId) {
      console.warn(
        '🚨 Invalid customer_id format detected:',
        profile.dodo_customer_id,
        'treating as new customer'
      )
    }
    ...
    console.log(
      '🚀 Sending subscription data to Dodo:',
    ...
    if (subscription.customer?.customer_id) {
      ...
      if (updateError) {
        ...
        console.log(
          `Stored Dodo customer_id ${subscription.customer.customer_id} for user ${userId}`
        )
      }
    }
    ...
    if (
      ...
      console.error(
        '🚨 Dodo API returned 404 - likely invalid customer_id or product_id'
      )
        ...
        return NextResponse.json(
          {
            ...
            details:
              'Your customer account has been reset. Please try subscribing again.',
            retryable: true,
          },
          { status: 409 }
        )
      ...
      return NextResponse.json(
        {
          ...
          details:
            'There may be an issue with your existing customer account. Please contact support.',
    ...
    return NextResponse.json(
      {
        ...
        userMessage:
          'Unable to create subscription. Please try again or contact support if the problem persists.',
      },
      { status: 500 }
    )
  }

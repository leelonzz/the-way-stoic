  ...
  try {
    ...
    if (!name || !template_content || !category || !icon) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: name, template_content, category, icon',
        },
        { status: 400 }
      )
    }
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to delete template or template not found' },
        { status: 404 }
      )
    }
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to save template to your collection' },
        { status: 500 }
      )
    }
